<template>
	<form class="content">
		...
		<div class="user_phone_wrap">
			...
			<input type="text" v-model="userphone" id="userphone" maxlength="11" placeholder="请输入手机号" />
		</div>
		<div class="validate">
		...
		<p class="changePhone" @click="tip()">立即换绑</p>
	</form>
</template>
...
export default{
	...
	mounted(){
		this.bindPhone = this.$store.state.bindPhone;
	},
	methods:{
		tip(){
			MessageBox({
...
//          };
//          //请求验证码接口
//			this.$http.get('http://www.xiangmulai.com/api/sms/'+this.userphone,{})
